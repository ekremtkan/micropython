...
def _wrap_strftime(object, format, timetuple):
    ...
    return  strftime(timetuple,newformat)
...
def strftime(time_t:tuple,fmt:str):
    ...
    rets=''
    fmt_list=fmt_split(fmt)
    for k in fmt_list.keys():
    ...
    def __hash__(self):
        ...
        if self._hashcode == -1:
            ...
            if not tzoff:  # zero or None
                ...
                h, m = divmod(timedelta(hours=self._hour, minutes=self._minute) - tzoff,
                              timedelta(hours=1))
                assert not m % timedelta(minutes=1), "whole minute"
                m //= timedelta(minutes=1)
                if 0 <= h < 24:
                    self._hashcode = hash(time(h, m, self._second, self._microsecond))
                else:
                    self._hashcode = hash((h, m, self._second, self._microsecond))
        return self._hashcode
...
class datetime(date):
    ...
    def _strptime(cls,time_t:str,fmt:str):
        Y,y,m,d,H,M,S,s=0,0,0,0,0,0,0,0
        t1,t2=0,0
        d_l,sp_l=date_split(time_t)
        no_separator=False
        if len(d_l) ==1:
            no_separator=True
        fmt_list=fmt_split(fmt)
        begin=0
        for i,k in enumerate(fmt_list.keys()):
            step=date_len(k)
            if no_separator:
    ...
    def strptime(cls, date_string, format):
        ...
        return eval('datetime%s'%str(cls._strptime(cls, date_string, format)[0:6]))
